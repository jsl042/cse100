...
public:
  ...
  {   
    BSTNode<Data>* search = root;
    /*iterator returned by end() called on the object*/
    iterator end = this->end();
     
    while(search)
    {     
      //if item larger than current node's key, go to right subtree 
      if(search->data < item)
      {
        if(search->right)
          search = search->right;
      ...
      //if item smaller than current node's key, go to left subtree 
      else if(item < search->data){
        if(search->left)
          search = search->left;
      ...
      //if not larger nor smaller, it's equal so return the node in an itr
      else
      {
        return typename BST<Data>::iterator(search);
      } 
